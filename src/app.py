...
def stock_prediction(ticker, year, percentile="p50"):
    ...
    predicted_value = fp['starting_price']*mult
...
def stock_trade_ep():
    ...
    shares_qty = float(d.get('shares', 0))
    cash = float(d.get('cash', 0))
...
# @app.route('/api/stock-prediction/<ticker>/<int:year>/<percentile>')
# def stock_prediction(ticker, year, percentile="p50"):
#     """Get predicted price multiplier for a stock at a given year."""
#     import json
#     from pathlib import Path
...
#         'growth_annual': data['estimated_yearly_growth'],
#         'risk_annual_volatility': data['risk_annual_volatility']
#     })
